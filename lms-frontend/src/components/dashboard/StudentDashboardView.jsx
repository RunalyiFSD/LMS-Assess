...
const StudentDashboardView = () => {
  ...
  const [viewMode, setViewMode] = useState('default'); // 'default' | 'explore_mocks'
    ...
    const fetchStudentData = async () => {
      ...
      try {
        ...
        if (topRes.data?.status === 'success') {
          let rankList = topRes.data.data.rankings || [];
          if (rankList.length < 3) {
            const mockTop = [
              { student: { name: 'Abhishek Sharma' }, totalScore: 181039 },
              { student: { name: 'Gopalsamy' }, totalScore: 151678 },
              { student: { name: 'Nikunj Shah' }, totalScore: 149479 }
            ];
            rankList = [...rankList, ...mockTop.slice(rankList.length)];
          }
          setTopThree(rankList);
        }
      } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="pt-6">
        ...
        <div className="text-center mt-6">
          <button
            onClick={() => setViewMode('explore_mocks')}
      ...
      {selectedScorecard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto print:p-0 print:bg-white print:static">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl flex flex-col border border-slate-100 print:border-none print:shadow-none print:max-w-full animate-in fade-in-50 duration-200">
            ...
            <div className="p-8 space-y-6 overflow-y-auto flex-1 print:p-0">
              {/* Institution Title */}
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between print:hidden rounded-b-2xl">
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
