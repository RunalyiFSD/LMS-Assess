...
const InstructorDashboardView = () => {
  ...
  const [showMockQuestionsModal, setShowMockQuestionsModal] = useState(false);
  const [mockEditForm, setMockEditForm] = useState({
    duration: 30,
    passingScore: 40,
    totalMarks: 100,
  });
  const [tempMockQuestions, setTempMockQuestions] = useState([]);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const mockRes = await api.get('/assessments?isMock=true');
  ...
  const handleOpenMockQuestions = async (mock) => {
    ...
    try {
      ...
      console.warn('Failed to load mock assessment details:', err);
      setTempMockQuestions(mock.questions || []);
    }
    setShowMockQuestionsModal(true);
  };
  ...
  const handleMockQuestionsSave = async () => {
    try {
      ...
      if (res.data?.status === 'success') {
        setShowMockQuestionsModal(false);
        fetchDashboardData();
      }
    } catch (err) {
      alert(err.message || 'Failed to update mock questions.');
    }
  };
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'assessments' && (
        <Card
          ...
          extra={
            <Button size="sm" onClick={() => setShowAssessmentModal(true)} className="gap-1">
                  ...
                  {mockAssessments.map((mock) => (
                    <tr key={mock._id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-xs flex gap-2">
                        ...
                        <Button size="sm" variant="primary" onClick={() => handleOpenMockQuestions(mock)}>
                          Edit Questions
                        </Button>
                      </td>
                    </tr>
      ...
      <Modal isOpen={showAssessmentModal} onClose={() => setShowAssessmentModal(false)} title="Create New Assessment">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 px-3 text-xs font-bold rounded-lg border transition-all ${
                      codingForm.difficulty === 'moderate'
                        ? 'border-amber-500 bg-amber-50 text-amber-700 shadow-sm'
                        : 'border-slate-200 text-slate-600 bg-white hover:bg-slate-50'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 px-3 text-xs font-bold rounded-lg border transition-all ${
                      codingForm.difficulty === 'difficult'
                        ? 'border-red-500 bg-red-50 text-red-700 shadow-sm'
                        : 'border-slate-200 text-slate-600 bg-white hover:bg-slate-50'
                    }`}
                  >
      ...
      <Modal isOpen={showMockEditModal} onClose={() => setShowMockEditModal(false)} title={`Edit Details - ${selectedMock?.title}`}>
      ...
      {/* Edit Mock Questions Modal */}
      <Modal isOpen={showMockQuestionsModal} onClose={() => setShowMockQuestionsModal(false)} title={`Edit Questions - ${selectedMock?.title}`}>
        <div className="space-y-4 max-h-[80vh] overflow-y-auto pr-2">
          {/* Current questions in mock */}
          <div>
            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Current Questions in Mock</h4>
            {tempMockQuestions.length === 0 ? (
              <p className="text-xs text-slate-400 bg-slate-50 p-3 rounded-lg border border-dashed">No questions added yet.</p>
            ) : (
              <div className="space-y-2">
                {tempMockQuestions.map((q, idx) => {
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {questions.map((q) => (
                <div key={q._id} className="flex items-center justify-between p-3 bg-white border rounded-lg text-xs hover:bg-slate-50/50">
                  ...
                  <button
                    type="button"
                    onClick={() => handleAddQuestionToMock(q)}
          ...
          <Button type="button" onClick={handleMockQuestionsSave} className="w-full mt-4">
